...
def _build_groups(turn_info: str) -> dict[str, list[dict]]:
    return {
        ...
        "github": [
            {"name": "fetch_github_repo", "description": "Browse GitHub. Workflow: search → info → get_tree (paginated via tree_offset/tree_limit) → find_string (±10 lines) → read_files (max 10, use line_ranges to limit). action: search/info/get_tree/find_string/read_files.", "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["search", "info", "get_tree", "tree", "read_files", "find_string"]}, "query": {"type": "string"}, "url": {"type": "string"}, "urls_list": {"type": "array", "items": {"type": "string"}}, "line_ranges": {"type": "array", "items": {"oneOf": [{"type": "array", "items": {"type": "integer"}, "minItems": 2, "maxItems": 2}, {"type": "null"}]}}, "tree_offset": {"type": "integer", "default": 0}, "tree_limit": {"type": "integer", "default": 200}}, "required": ["action"]}},
        ],
