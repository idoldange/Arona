...
def _build_groups(turn_info: str) -> dict[str, list[dict]]:
    return {
        ...
        "media": [
            {"name": "reverse_image_search", "description": "Find image source by URL. `image_url` MUST be the exact URL copied verbatim from the [Attachment: <filename> | URL: <url>] tag in the current message parts — character-for-character. If no such tag exists in the current message, do not call this tool. Never construct, guess, recall from memory, or modify any URL. backend: googlelens=general photos/products/people (default), saucenao=anime/manga/illustration art source lookup.", "parameters": {"type": "object", "properties": {"image_url": {"type": "string"}, "backend": {"type": "string", "enum": ["googlelens", "saucenao"], "default": "googlelens"}, "crawl_per_query": {"type": "integer", "default": 3}, "max_chars_per_page": {"type": "integer", "default": 10000}}, "required": ["image_url"]}},
