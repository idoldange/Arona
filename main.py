    ...
    if function_name == "web_search":
      ...
      try:
        ...
        if message and (has_output_files or not send_output):
            asyncio.create_task(send_and_cleanup_code_outputs(message, workspace_key, send_output=send_output, send_code=send_code, send_logs=send_logs, is_temp=False))
        if temp and message:
...
async def send_and_cleanup_code_outputs(message: discord.Message, msg_id: str, send_output: bool = True, send_code: bool = False, send_logs: bool = False, is_temp: bool = True):
    """
